...
const Visibility = () => {
    const {visibility} = useAddPostInfo()
            ...
            <div className="flex flex-col gap-2 items-center justify-center w-full">
                <div className="flex text-xl flex-row gap-4 items-center justify-start w-full">
                    ...
                    <select value={visibility.value} onChange={onValueChange} className="p-2 bg-white text-black">
                        <option value={PostVisibilityEnum[PostVisibilityEnum.PUBLIC]}>PUBLIC</option>
                        <option value={PostVisibilityEnum[PostVisibilityEnum.RESTRICTED]}>RESTRICTED</option>
                        <option value={PostVisibilityEnum[PostVisibilityEnum.PRIVATE]}>PRIVATE</option>
                    </select>
                </div>
                <span className="italic text-sm w-full">
                {
                    visibility.value === PostVisibilityEnum[PostVisibilityEnum.PUBLIC] ?
                        "Everyone will be able to see your post" :
                        visibility.value === PostVisibilityEnum[PostVisibilityEnum.RESTRICTED] ?
