...
class Nft {
    ...
    public async removeNftBackupFromDb() {
        await prisma.nft_backup.delete({
            where: {
                ipfs: this.ipfs
            }
        })
    }
    public static async getNftByIpfs(ipfs: string) {
    ...
    public async checkIfNftVerified(address: string) {
        let nftId: string
        try{
            nftId = (await Nft.socialNftContract.getNftId(address, this.ipfs)).toString()
        }catch (e) {
        ...
        await this.removeNftBackupFromDb()
    }
